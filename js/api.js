    ...
    try {
        ...
        for (const bus of buses) {
            ...
            const arrowPath = hasValidBearing ? (() => {
                ...
                const tipX = 19 + Math.sin(angle) * L;
                const tipY = 19 - Math.cos(angle) * L;
                const leftX = 19 + Math.sin(angle + Math.PI / 2) * W;
                const leftY = 19 - Math.cos(angle + Math.PI / 2) * W;
                const rightX = 19 + Math.sin(angle - Math.PI / 2) * W;
                const rightY = 19 - Math.cos(angle - Math.PI / 2) * W;
                return `<polygon points="${tipX},${tipY} ${leftX},${leftY} ${rightX},${rightY}" fill="${color}" opacity="0.95"/>`;
            })() : "";

            const svgSize = 38;
            const iconHtml = `
                <svg xmlns="http://www.w3.org/2000/svg" width="${svgSize}" height="${svgSize}" viewBox="0 0 ${svgSize} ${svgSize}" style="overflow:visible;filter:drop-shadow(0 2px 4px rgba(0,0,0,0.5));">
                    ${arrowPath}
...
function updateVisibleParkings() {
    ...
    visible.forEach(p => {
        ...
        const m = L.marker([parseFloat(p.lat), parseFloat(p.lon)], {
            ...
            icon: L.divIcon({
                html: `
                    <div style="position:relative;width:28px;height:28px;filter:drop-shadow(0 2px 4px rgba(0,0,0,0.5));">
...
// ============================================
// CHARGEMENT DE LA LISTE DES VÉLO'V
// ============================================

/**
 * Charge la liste des stations Vélo'v.
 */
...
// ============================================
// FILTRAGE DES STATIONS VÉLO'V
// ============================================

let velovSearchTimer = null;

/**
 * Filtre les stations Vélo'v en fonction de la recherche.
 */
...
/**
 * Affiche la liste des stations Vélo'v.
...
function renderVelovList(term) {
    ...
    const html = shown.map(s => {
        ...
        return `
            <div class="info-item" data-lat="${s.lat || ''}" data-lon="${s.lng || ''}" data-name="${(s.name || '').replace(/"/g, '&quot;')}" data-idx="${allVelovStations.indexOf(s)}">
...
// ============================================
// SÉLECTION D'UNE STATION VÉLO'V SUR LA CARTE
// ============================================

/**
 * Sélectionne une station Vélo'v sur la carte.
 */
