...
async function updateBus() {
    try {
        ...
        for (const bus of buses) {
            ...
            const arrowPath = (() => {
                if (!hasValidBearing) return "";
                const angle = bearing * Math.PI / 180, L = 26, W = 12;
                const tipX = 19 + Math.sin(angle) * L, tipY = 19 - Math.cos(angle) * L;
                const leftX = 19 + Math.sin(angle + Math.PI / 2) * W, leftY = 19 - Math.cos(angle + Math.PI / 2) * W;
                const rightX = 19 + Math.sin(angle - Math.PI / 2) * W, rightY = 19 - Math.cos(angle - Math.PI / 2) * W;
            ...
            const iconHtml = `<svg xmlns="http://www.w3.org/2000/svg" width="${svgSize}" height="${svgSize}" viewBox="0 0 ${svgSize} ${svgSize}" style="overflow:visible;filter:drop-shadow(0 2px 4px rgba(0,0,0,0.5));">
                ${arrowPath}
                <rect x="3" y="3" width="32" height="32" rx="8" fill="rgba(13,15,24,0.9)" stroke="${color}" stroke-width="2"/>
                <image href="assets/Lignes/${modeFile}" x="9" y="9" width="18" height="18"/>
            </svg>`;
        ...
        // Supprimer les bus qui n'existent plus
...
// Mettre à jour les parkings visibles
function updateVisibleParkings() {
    ...
    visible.forEach(p => {
        ...
        const m = L.marker([parseFloat(p.lat), parseFloat(p.lon)], {
            ...
            icon: L.divIcon({
                html: `<div style="position:relative;width:28px;height:28px;filter:drop-shadow(0 2px 4px rgba(0,0,0,0.5));">
                    <span style="font-size:22px;display:flex;align-items:center;justify-content:center;width:28px;height:28px;background:rgba(13,15,24,0.9);border-radius:6px;">🅿️</span>
        ...
        }).bindPopup(buildParkingPopup(p, null, 'Parking', '🅿️'), { maxWidth: 240, className: 'tcl-popup' }).addTo(parkingLayer);
...
// Mettre à jour les parcs relais visibles
function updateVisibleParkAndRideLots() {
    ...
    visible.forEach(p => {
        ...
        }).bindPopup(buildParkingPopup(p, null, 'Parc Relais TCL', '🅿️'), { maxWidth: 240, className: 'tcl-popup' }).addTo(parcsRelaisLayer);
...
// Mettre à jour les Vélo'v visibles
    ...
    try {
        ...
        allAgences.forEach(a => {
            ...
            L.marker([a.lat, a.lon], {
                icon: L.divIcon({
                    html: `<div style="width:36px;height:36px;background:linear-gradient(135deg, #ff4d4d, #e2001a);border-radius:10px;border:2px solid rgba(255,255,255,0.2);display:flex;align-items:center;justify-content:center;box-shadow:0 4px 12px var(--accent-glow);">
...
async function updateTraffic() {
    try {
        ...
        if (!items.length) {
            list.innerHTML = `<div class="info-empty">
...
// Charger les alertes accessibilité
...
// Charger la liste des Vélo'v
async function loadVelovList() {
    try {
        const data = await apiFetch('api/velov');
        if (data.is_loading) {
            setTimeout(loadVelovList, 10000);
            return;
        }
        allVelovStations = data.values || [];
        renderVelovList('');
        console.log("✅ Liste Vélo'v chargée");
    } catch (e) {
        setTimeout(loadVelovList, 10000);
    }
}

// Filtrer les stations Vélo'v
...
function renderVelovList(term) {
    ...
    const html = shown.map(s => {
        ...
        return `<div class="info-item" data-lat="${s.lat || ''}" data-lon="${s.lng || ''}" data-name="${(s.name || '').replace(/"/g, '&quot;')}" data-idx="${allVelovStations.indexOf(s)}">
            <div class="info-summary" onclick="selectVelovOnMap(this)">
                <div class="info-line-badge" style="background:rgba(${dispoInfo.rgb},0.12);border:1px solid rgba(${dispoInfo.rgb},0.25);">
                    <img src="assets/SVG_Icons/velo.svg" class="svg-ic" style="width:18px;height:18px;">
                </div>
                <div class="info-summary-text">
    ...
    document.getElementById('velov-list').innerHTML = html +
        (stations.length > 300 ? `<div class="info-empty">+${stations.length - 300} stations (affinez)</div>` : '');
}

// Sélectionner une station Vélo'v sur la carte
function selectVelovOnMap(el) {
    ...
    const station = allVelovStations[parseInt(item.dataset.idx)] || {
...
// Charger les prochains départs pour un arrêt
...
async function loadNextDepartures(stopId, containerId, stopLat, stopLon) {
    ...
    try {
        ...
        if (data.is_loading) {
            ...
            setTimeout(() => loadNextDepartures(stopId, containerId, stopLat, stopLon), 10000);
            return;
        }
...
// Rendre les arrêts sur la carte
...
function renderVisibleStops() {
    ...
    const bounds = map.getBounds();
    const seen = new Map();
    const visible = stopsOnMap.filter(s => bounds.contains([s.lat, s.lon])).slice(0, 500);
...
async function openStopGroupPopup(stopGroup) {
    ...
    if (main.id) loadNextDepartures(main.id, `deps-${safeId}`, stopGroup.lat, stopGroup.lon);
}
...
// Appliquer le filtre de ligne aux bus
...
function renderStopList(term = '') {
    ...
    if (!stops.length) {
        document.getElementById('stops-list').innerHTML = `<div class="info-empty">
...
// Sélectionner un arrêt sur la carte
...
// Rechercher une ligne dans les arrêts
...
// Filtrer une ligne depuis un popup
...
// Obtenir les lignes pour un arrêt
