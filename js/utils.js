...
// ============================================
window.imgCache = new Map();

// Couleurs des lignes (mappage couleur → lignes)
window.COLOR_TO_LINES = {
...
window.LINE_COLORS_MASTER = (() => {
    const master = {};
    for (const [color, lines] of Object.entries(window.COLOR_TO_LINES)) {
...
window.getLineColor = function(code) {
    if (!code) return window.LINE_TYPE_COLORS.other;
    const u = String(code).trim();
    if (window.LINE_COLORS_MASTER[u]) return window.LINE_COLORS_MASTER[u];
    if (window.LINE_COLORS_MASTER[u.toUpperCase()]) return window.LINE_COLORS_MASTER[u.toUpperCase()];
    return window.LINE_TYPE_COLORS[window.getLineType(u)] || window.LINE_TYPE_COLORS.other;
};

window.getLineType = function(line) {
...
window.lineImgHtml = function(lineNum, height = '20px') {
    ...
    const color = window.getLineColor(lineNum);

    const state = window.imgCache.get(src);
    ...
    return `<img src="${src}" data-line="${lineNum}" style="height:${height};width:auto;display:inline-block;vertical-align:middle;" loading="lazy"
        onload="window.imgCache.set(this.src,'ok')"
        onerror="window.imgCache.set(this.src,'error');var s=document.createElement('span');s.textContent=this.dataset.line;s.style.cssText='${fallbackStyle}color:${color};';this.replaceWith(s)">`;
};

window.getDispoColor = function(value, total) {
...
window.getVelovDispoColor = function(bikes, stands, total) {
    ...
    return window.getDispoColor(bikes, total || (bikes + stands));
};
...
window.fmtDepAt = function(isoStr, now) {
    if (!isoStr) return '—';
...
window.getNewLineNumber = function(l) {
    return window.lineMapping[l] || l;
};

window.getOriginalCodes = function(targetMapped) {
    const codes = new Set([targetMapped]);
    for (const [orig, mapped] of Object.entries(window.lineMapping)) {
        if (mapped === targetMapped) codes.add(orig);
    }
    return codes;
};

window.stopServesLine = function(desserte, targetMapped) {
    if (!desserte) return false;
    const codes = window.getOriginalCodes(targetMapped);
    return desserte.split(',').some(entry => {
        const code = entry.trim().split(':')[0].trim();
        return codes.has(code) || codes.has(window.getNewLineNumber(code));
    });
};

window.extractLines = function(desserte) {
    if (!desserte) return [];
    const lines = [...new Set(
        desserte.split(',').map(x => window.getNewLineNumber(x.trim().split(':')[0].trim()))
    )];
    return lines.sort(window.sortLinesByType);
};

window.renderLineSvgs = function(lines) {
    if (!lines?.length) return '<span style="color:var(--text-muted);font-size:11px;">Aucune ligne</span>';
    return lines.map(l => window.lineImgHtml(window.getNewLineNumber(l), '20px')).join('');
};

window.getLineTypePriority = function(type) {
    const priorities = {
...
window.sortLinesByType = function(a, b) {
    const typeA = window.getLineType(a);
    const typeB = window.getLineType(b);
    const priorityA = window.getLineTypePriority(typeA);
    const priorityB = window.getLineTypePriority(typeB);
...
window.getVehicleType = function(bus) {
...
window.busHash = function(b) {
    return `${b.lat},${b.lon},${b.bearing},${b.delay},${b.dest_code}`;
};

window.getStopName = function(code) {
    if (!code) return null;
    const clean = code.includes(':') ? code.split(':').pop() : code;
    return window.stopsMapping[clean] || window.stopsMapping[code] || null;
};
...
window.buildBusPopup = function(bus) {
    return `
...
window.buildAgencyPopup = function(a, adresse, facea) {
    return `
...
window.buildVelovPopup = function(s) {
...
window.toggleInfoItem = function(element) {
...
window.switchPanelTab = function(tabName) {
    window.currentPanelTab = tabName;
    ...
    if (tabName === 'velov' && !window.velovLoaded) {
        window.velovLoaded = true;
        window.updateVelov();
    }
};

window.filterVelovStations = function() {
    const term = document.getElementById('velov-search').value.toLowerCase().trim();
    console.log("Filtre Vélo'v :", term);
};

window.updateIconSizes = function() {
    const zoom = window.map.getZoom();
    // Logique de redimensionnement des icônes si nécessaire
};
...
window.clearAllFilters = function() {
    ...
    if (typeof window.updateVisibleVelov === 'function') window.updateVisibleVelov();
    if (typeof window.updateVisibleParkings === 'function') window.updateVisibleParkings();
    if (typeof window.updateVisibleParkAndRideLots === 'function') window.updateVisibleParkAndRideLots();
    if (typeof window.renderVisibleStops === 'function') window.renderVisibleStops();
    if (typeof window.applyBusLineFilter === 'function') window.applyBusLineFilter();
};

window.filterLineFromPopup = function(line) {
    window.busLineFilter = window.getNewLineNumber(line);
    window.currentLineFilter = window.busLineFilter;
...
// ============================================
// FONCTION MANQUANTE - CHECK SYSTEM STATUS
// ============================================
window.checkSystemStatus = async function() {
    ...
    try {
        ...
        // En cas d'erreur, on affiche "online" pour éviter les erreurs console
        statusDot.className = 'system-status-dot online';
        statusDot.title = 'Mode dégradé';
    }
};
